#!/usr/bin/env python3
"""Import SigmaHQ threat-hunting rules into upstream/<category>/, and write sigma_tracker.md.
...
huntgraph templates: validate.py does not scan upstream/, and nothing here is converted.
...
def main() -> int:
    ...
    for folder, _, _ in CATEGORIES:
        shutil.rmtree(os.path.join(UPSTREAM, folder, "sigma"), ignore_errors=True)

    rows, counts = [], collections.Counter()
    for p in paths:
        rule = yaml.safe_load(bodies[p]) or {}
        cat = categorise(rule)
        rel = os.path.join("upstream", cat, "sigma", os.path.relpath(p, "rules-threat-hunting"))
    ...
    for folder, name, meaning in CATEGORIES:
        ...
        with open(os.path.join(d, "README.md"), "w") as fh:
            fh.write("# %s\n\nUpstream rules that read %s. Category as defined by Elastic's "
                     "[prebuilt rules](%s%s).\n\n" % (name, meaning, ELASTIC, folder))
            fh.write("Sigma rules here: %d. " % counts[folder] if counts[folder] else "No rules yet. ")
            fh.write("See [sigma_tracker.md](../../sigma_tracker.md).\n\nGenerated by "
                     "`python3 tools/import_sigma.py` — regenerate, never hand-edit.\n")
    ...
    out = ["# Sigma threat-hunting rule tracker\n",
           ...
           "Rules are stored unmodified under `upstream/<category>/sigma/`. They are upstream "
           "material, not huntgraph templates, and are not validated.\n",
           "Generated by `python3 tools/import_sigma.py` — regenerate, never hand-edit.\n",
           "## By category\n", "| Category | Rules |", "|---|---:|"]
    out += ["| [%s](upstream/%s/) | %d |" % (name, folder, counts[folder]) for folder, name, _ in CATEGORIES]
