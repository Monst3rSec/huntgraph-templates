...
_AGGREGATES = re.compile(r"\bgroupBy\(")
...
def check_query_emits_declared(doc, rep: Report) -> None:
    ...
    A case with no groupBy returns whole events, so it hands the agent every
    declared field of each log source whose event types it queries. Only an
    aggregating case has to name its fields, because only there can one be
    filtered on and then silently dropped.
    """
    declared = {}
    cs_logs = []
    for log in doc["requires"]["logs"]:
        if log_connector(log) != "crowdstrike-ngsiem":
            continue
        fields = log["fields"]
        vendor = list(fields.values() if isinstance(fields, dict) else fields)
        cs_logs.append((log["event_types"], vendor))
        for name in vendor:
    ...
    emitted, queried = set(), set()
    for block in doc["query"]:
        if block["platform"] == "wazuh":
            continue
        for case in block["cases"]:
            q = case["query"]
            # a case can only surface fields carried by the event types it reads;
            # naming a file field in a collect() over process events returns nothing
            readable = set()
            for event_types, vendor in cs_logs:
                if any(e in q for e in event_types):
                    readable |= set(vendor)
            queried |= readable
            emitted |= (emitted_fields(q) & readable) if _AGGREGATES.search(q) else readable

    missing = set(declared) - emitted
    dropped = sorted(missing & queried)
    if dropped:
        rep.error("L4", "requires.logs declares %s which no cql case returns; a declared "
                        ...
                        "cannot reason over" % ", ".join(repr(m) for m in dropped))
    unread = {}
    for event_types, vendor in cs_logs:
        lost = sorted(set(vendor) & (missing - queried))
        if lost:
            unread.setdefault(", ".join(event_types), set()).update(lost)
    for events, lost in sorted(unread.items()):
        rep.warn("L4", "requires.logs declares %s from %s, but no cql case queries those "
                       "event types; the hunt declares telemetry it never collects"
                 % (", ".join(repr(f) for f in sorted(lost)), events))
