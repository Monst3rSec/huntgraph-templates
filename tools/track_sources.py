...
def write_tracker(path, title, source_url, rows, base_url):
    ...
    # a rule is converted when a template cites it, whatever its technique now routes to:
    # authoring the template usually covers the technique, which moves the rule to `covered`
    converted = [r for r in rows if r.get("template")]
    queue = counts.get("convert", 0)
    remaining = sum(1 for r in rows if r["decision"] == "convert" and not r.get("template"))
    out = []
    out.append("# %s\n" % title)
    ...
    out.append("**%d rules tracked · %d queued for conversion · %d converted · %d remaining**\n"
               % (len(rows), queue, len(converted), remaining))
    out.append("## Routing\n")
    ...
    out.append("")
    if converted:
        out.append("## converted — %d\n" % len(converted))
        out.append("_rules a template cites in `info.references`; the template is the conversion_\n")
        out.append("| Rule | Technique | Template | Source |")
        out.append("|---|---|---|---|")
        for r in sorted(converted, key=lambda x: x["template"]):
